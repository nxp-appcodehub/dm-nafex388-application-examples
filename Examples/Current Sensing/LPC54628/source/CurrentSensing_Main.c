...
int main(void)
{
    ...
    NAFESDK_WriteReg16B(&instance, CH_CONFIG0, 0x4430); // AI4P to AI4N - GAIN 0.4x
    NAFESDK_WriteReg16B(&instance, CH_CONFIG1, 0x1028); // SINC4 - 12000Sps
    NAFESDK_WriteReg16B(&instance, CH_CONFIG2, 0x4400); // Delay 16.5us
    gains[0]=1;
    ...
    while(1)
    {
    	...
    	// For Cycle for each channel
    	for (int i=0;i<1;i++){
			NAFESDK_CmdSetCurrentPointer(&instance,i); // Set Pointer to ch i (0 -> 0)
			...
			while(GPIO_PinRead(GPIO, instance.gpio_rdy_port, instance.gpio_rdy_pin)==0); /* wait for RDY pin high */
    	...
    	PRINTF("------------Conversion-------------\r\n");
    	PRINTF("\r\nVoltage Shunt=%6fV, Current Shunt(0.25ohm)=%6fmA\r\n", voltages[0],(voltages[0]/0.25)*1000);
    	time_tot = 0;
    	for (int i=0;i<1;i++){
