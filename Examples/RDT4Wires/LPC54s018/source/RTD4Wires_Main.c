...
int main(void)
{
    ...
    NAFESDK_WriteReg16B(&instance, CH_CONFIG3, 0xB010); // Current - Positive Pol - 750uA - AI2P
    gain=4;
    ...
    while(1)
    {
	    ...
	    for(i=0; i<len / ADC_READ_SIZE; i++)
	    {
			...
			voltageSum += voltage;
			PRINTF("Sample %d: Voltage RTD=%6fV, Temperature RTD=%3fC\r\n", i,voltage,((voltage/ISource)-R0)/(A*R0)); //Calculation of Temperature.
	    }
	    PRINTF("-----------------------------------");
	    voltage = voltageSum/conversionNumber; //Average Calculation
    	PRINTF("\r\n------------Average Conversion-------------");
    	PRINTF("\r\nVoltage RTD=%6fV, Temperature RTD=%3fC", voltage,((voltage/ISource)-R0)/(A*R0)); //Calculation of Temperature.
    	PRINTF("\r\n-----------------------------------\r\n");
    }
