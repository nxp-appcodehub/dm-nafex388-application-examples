...
int main(void)
{
    ...
    NAFESDK_WriteReg16B(&instance, CH_CONFIG0, 0x11F1); // AI1P to AI1N - GAIN 16x
    NAFESDK_WriteReg16B(&instance, CH_CONFIG1, 0x70A4); // SINC4+1 - 100Sps
    NAFESDK_WriteReg16B(&instance, CH_CONFIG2, 0x2C80); // Delay 16.5us
    NAFESDK_WriteReg16B(&instance, CH_CONFIG3, 0x3410); // Voltage - Positive Pol - 6V - AI2P
    gain=16;

	PRINTF("-> Calculation of Coefficient <-\r\n");
	...
	while (confirmation){
		...
		PRINTF("The weight on the scale is %dg, do you confirm(y/n)?\r\n", knownWeight);
		key = GETCHAR();
...

    {
		...
		voltage = voltage/conversionNumber; //Average Calculation
		PRINTF("Voltage Load Cell=%6fmV, Weight=%1fg\r\n", voltage*1000,coefficient*(voltage-offset)); //Calculation of Temperature.
