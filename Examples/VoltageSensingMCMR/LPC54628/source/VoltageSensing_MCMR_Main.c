...
int main(void)
{
    ...
    NAFESDK_WriteReg16B(&instance, CH_CONFIG4, chlmask);
    ...
    while(1)
    {
		...
		startMcmr(&instance, chlmask, 0);
		while(NAFESDK_GetReadingsRemaining(&instance) != 0) {};
		NAFESDK_CmdAbort(&instance);
		len = NAFESDK_GetRBCount(&instance);

	    PRINTF("\r\n------------Conversion-------------\r\n");
	    for(int i=0; i<len / ADC_READ_SIZE; i++)
	    {
